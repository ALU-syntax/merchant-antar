...
import com.antar.merchant.json.MobilePulsaHealthBPJSResponseModel;
...
public class SettingPreference {
    ...
    public void updatePlnList(TopUpRequestResponse model) {
        editor = pref.edit();
        List<TopUpRequestResponse> listModel = new ArrayList<>();
        String previousStringModel = pref.getString(PLN_LIST_PAID_SUCCESS,"");
        if (!previousStringModel.isEmpty()) {
            Type listOfMyClassObject = new TypeToken<ArrayList<TopUpRequestResponse>>() {}.getType();
    ...
    public String[] getSetting() {
        ...
        settingan[13] = pref.getString(KEY_NOTIF, "ON");
