...
import com.bumdesku.merchant.models.FiturModel;
import com.bumdesku.merchant.models.KategoriModel;
import com.bumdesku.merchant.models.Wilayah;
import com.bumdesku.merchant.utils.Log;
...
public class RegisterActivity extends AppCompatActivity {
    ...
    Spinner merchantype, merchantcat, wilayahtype;
    String phoneNumber;
    ...
    ArrayList<Wilayah> wilayahlist;
    ArrayList<String> jobsdata;
    ArrayList<String> cabdata;
    private List<FiturModel> fiturdata;
    private List<Wilayah> wilayahdata;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        etreferal = findViewById(R.id.et_referal);
        ...
        wilayahtype = findViewById(R.id.wilayahtype);
    ...
    private void spinner3() {
        Wilayah wilayahmodel = new Wilayah();
        wilayahmodel.setNama_cabang("0");
        wilayahmodel.setId(-1);
        wilayahlist.add(wilayahmodel);
        cabdata.add("Pilih Wilayah");
        WilayahRequestJson param = new WilayahRequestJson();
        param.setWilayah(121);
        MerchantService userService = ServiceGenerator.createService(MerchantService.class,
                "admin", "admin");
        userService.getWilayah(param).enqueue(new Callback<WilayahResponseJson>() {
            @Override
            public void onResponse(Call<WilayahResponseJson> call, Response<WilayahResponseJson> response) {
                if (response.isSuccessful()) {
                    wilayahdata = Objects.requireNonNull(response.body()).getData();
                    for (int i = 0; i < wilayahdata.size(); i++) {
                        Wilayah wils = new Wilayah();
                        wils.setNama_cabang(wilayahdata.get(i).getNama_cabang());
                        wils.setId(wilayahdata.get(i).getId());
                        wilayahlist.add(wils);
                        cabdata.add(wilayahdata.get(i).getNama_cabang());
                    }

                    ArrayAdapter<String> wilayahSpinner = new ArrayAdapter<>(RegisterActivity.this, R.layout.spinner, cabdata);
                    wilayahSpinner.setDropDownViewResource(R.layout.spinner);
                    wilayahtype.setAdapter(wilayahSpinner);
                    wilayahtype.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                        ...
                        public void onItemSelected(AdapterView<?> parent, View view,
                            ...
                            if (position == 0) {
                                ...
                                int idwilayah = wilayahlist.get(wilayahtype.getSelectedItemPosition()).getId();
                                spinner(idwilayah);
            ...
            @Override
            public void onFailure(Call<WilayahResponseJson> call, Throwable t) {

            }
        });
    }

    private void spinner(int idwilayah) {
        ...
        GetFiturRequestJson param = new GetFiturRequestJson();
        param.setWilayah(idwilayah);
        MerchantService userService = ServiceGenerator.createService(MerchantService.class,
                "admin", "admin");
        userService.getFitur(param).enqueue(new Callback<GetFiturResponseJson>() {
            @Override
                    ...
                    merchantype.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                        ...
                        public void onItemSelected(AdapterView<?> parent, View view,
                            ...
                            if (position == 0) {
                                ((TextView) parent.getChildAt(0)).setTextColor(getResources().getColor(R.color.gray));
                                ((TextView) parent.getChildAt(0)).setTextSize(14);
                                spinner2(String.valueOf(fiturlist.get(merchantype.getSelectedItemPosition()).getIdFitur()), idwilayah);
                            } else {
                                ((TextView) parent.getChildAt(0)).setTextColor(getResources().getColor(R.color.black));
                                ((TextView) parent.getChildAt(0)).setTextSize(14);
                                spinner2(String.valueOf(fiturlist.get(merchantype.getSelectedItemPosition()).getIdFitur()), idwilayah);
    ...
    private void spinner2(String id, int idwilayah) {
        if (jobskatdata != null) {
            jobskatdata.clear();
        }
        if (katlist != null) {
    ...
    public void Nextbtn(View view) {
        ...
        if ((!TextUtils.isEmpty(phoneNumber) && !TextUtils.isEmpty(ccode)) && phoneNumber.length() > 5) {
            ...
            notif("Cek kembali nomor hp anda, mungkin salah");
        }
    }
    ...
    private void upload(final String check) {
        ...
        request.setReferal_code(etreferal.getText().toString());
        request.setWilayah(wilayahlist.get(wilayahtype.getSelectedItemPosition()).getId());
